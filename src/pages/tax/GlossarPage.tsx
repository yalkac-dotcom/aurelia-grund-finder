  ...
  return (
    <Layout>
      ...
      <section className="py-20 md:py-28">
        <div className="container max-w-3xl">
          <Reveal>
            <p className="text-accent font-sans text-[11px] font-medium tracking-[0.16em] uppercase mb-6">{t.tax.categoryKnowledge}</p>
          ...
          <Reveal delay={0.1}>
            <p className="text-muted-foreground text-[0.95rem] leading-[1.95] max-w-xl mb-12">{t.tax.glossarSectionSubtitle}</p>
          </Reveal>
          <div className="divide-y divide-border/60">
            {t.tax.glossarItems.map((g, i) => (
              <Reveal key={g.term} delay={i * 0.05}>
                <div className="grid md:grid-cols-[1fr_2fr] gap-3 md:gap-12 py-8 first:pt-0 last:pb-0">
                  <dt className="text-[0.88rem] font-heading font-semibold text-foreground">{g.term}</dt>
                  <dd className="text-muted-foreground text-[0.85rem] leading-[1.85]">{g.definition}</dd>
                </div>
              </Reveal>
            ))}
          </div>
